...
function Write-VersionFile {
    ...
    # Prefer the BuildInfo generated by the GenerateBuildInfo MSBuild target so the
    # root version.json carries the exact same Build GUID as Content\version.json.
    $buildInfoPath = Join-Path $PublishDir "Content\version.json"
    if (Test-Path -LiteralPath $buildInfoPath) {
        Copy-Item -LiteralPath $buildInfoPath -Destination $VersionPath -Force
        Write-Host "Wrote version.json (from BuildInfo)"
        return
    }

    $Warnings.Add("Content\version.json missing from publish output; wrote fallback version.json")
    $payload = [ordered]@{
