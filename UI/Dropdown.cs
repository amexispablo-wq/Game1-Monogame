...
public sealed class Dropdown<T> where T : notnull
{
    public string Label { get; set; } = string.Empty;
    public Rectangle Bounds { get; set; }
    public List<T> Options { get; } = new();
    public Func<T, string> DisplayFunc { get; set; } = value => value?.ToString() ?? string.Empty;
    public bool IsExpanded { get; set; }
    public int? HighlightedIndex { get; set; }
    public int DropdownHeight { get; set; } = 180;
    ...
    public T? SelectedOption
    {
        ...
        set
        {
            _selectedIndex = value is null ? -1 : Options.IndexOf(value);
        }
    }
    ...
    public void Draw(SpriteBatch spriteBatch, Texture2D pixel)
    {
        ...
        {
            SimpleTextRenderer.DrawString(spriteBatch, pixel, Label, new Vector2(Bounds.X, Bounds.Y - 24), 2, Color.White);
        }
    ...
    private void DrawHeader(SpriteBatch spriteBatch, Texture2D pixel)
    {
        ...
        string displayText = SelectedOption is null ? "Select" : DisplayFunc(SelectedOption);
    ...
    private void DrawDropdown(SpriteBatch spriteBatch, Texture2D pixel)
    {
        ...
        for (int i = 0; i < Options.Count; i++)
        {
            ...
            Color itemBg = isHighlighted ? new Color(74, 96, 132) : (isSelected ? new Color(55, 74, 102) : new Color(38, 48, 68));
            ...
            DrawFittedCentered(spriteBatch, pixel, DisplayFunc(Options[i]), itemBounds, 2, textColor);
        }
    }
    ...
    private int GetDropdownHeight() => Options.Count * GetItemHeight();
